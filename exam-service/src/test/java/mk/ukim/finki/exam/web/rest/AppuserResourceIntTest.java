...
import mk.ukim.finki.exam.service.AppuserQueryService;
...
public class AppuserResourceIntTest {
    ...
    @Test
    @Transactional
    public void getAllAppusersByExamIsEqualToSomething() throws Exception {
        // Initialize the database
        Exam exam = ExamResourceIntTest.createEntity(em);
        em.persist(exam);
        em.flush();
        appuser.addExam(exam);
        appuserRepository.saveAndFlush(appuser);
        Long examId = exam.getId();

        // Get all the appuserList where exam equals to examId
        defaultAppuserShouldBeFound("examId.equals=" + examId);

        // Get all the appuserList where exam equals to examId + 1
        defaultAppuserShouldNotBeFound("examId.equals=" + (examId + 1));
    }
