...
import mk.ukim.finki.gradespreview.repository.StudentRepository;
import mk.ukim.finki.gradespreview.service.StudentService;
import mk.ukim.finki.gradespreview.web.rest.errors.ExceptionTranslator;
import mk.ukim.finki.gradespreview.service.dto.StudentCriteria;
import mk.ukim.finki.gradespreview.service.StudentQueryService;
...
public class StudentResourceIntTest {
    ...
    @Test
    @Transactional
    public void getAllStudentsByCompletedCourseIsEqualToSomething() throws Exception {
        // Initialize the database
        CompletedCourse completedCourse = CompletedCourseResourceIntTest.createEntity(em);
        em.persist(completedCourse);
        em.flush();
        student.addCompletedCourse(completedCourse);
        studentRepository.saveAndFlush(student);
        Long completedCourseId = completedCourse.getId();

        // Get all the studentList where completedCourse equals to completedCourseId
        defaultStudentShouldBeFound("completedCourseId.equals=" + completedCourseId);

        // Get all the studentList where completedCourse equals to completedCourseId + 1
        defaultStudentShouldNotBeFound("completedCourseId.equals=" + (completedCourseId + 1));
    }
